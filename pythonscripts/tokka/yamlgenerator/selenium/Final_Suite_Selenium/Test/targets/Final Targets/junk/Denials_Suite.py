...
from tests.RCR_VR_HA_FE_ConfigTool_Denials import DenialsFE
...
if __name__ == '__main__':
    ...
    tabList = inputs['denialsTabList']
    denialsList = inputs['denialsList']
    ...
    if domain == 'staging':
        ...
        for incr in denialsList:
            denialsUrl = stagingBaseUrl + incr
            """Sending the parameters with parameterized class."""
            suite.addTest(ParametrizedTestCase.parametrize(DenialsFE, driverParam=browser, urlParam=denialsUrl, facility_name=facility_name, urlListParam = tabList))
    elif domain == 'dev':
        browser = login.login_dev()
        devBaseUrl = inputs['devBaseUrl']
        facility_name = inputs['facility_name_dev']
        for incr in denialsList:
            denialsUrl = devBaseUrl + incr
            """Sending the parameters with parameterized class."""
            suite.addTest(ParametrizedTestCase.parametrize(DenialsFE, driverParam=browser, urlParam=denialsUrl, facility_name=facility_name, urlListParam = tabList))
