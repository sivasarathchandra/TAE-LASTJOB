...
from tests.RCR_VR_HA_FE_ConfigTool_Census import CensusFE
...
if __name__ == '__main__':
    ...
    tabList = inputs['censusTabList']
    censusList = inputs['censusList']
    ...
    if domain == 'staging':
        ...
        for incr in censusList:
            adjustmentsUrl = stagingBaseUrl + incr
            """Sending the parameters with parameterized class."""
            suite.addTest(ParametrizedTestCase.parametrize(CensusFE, driverParam=browser, urlParam=adjustmentsUrl, facility_name=facility_name, urlListParam = tabList))
    elif domain == 'dev':
